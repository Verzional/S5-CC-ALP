    ...
    <div class="max-w-3xl mx-auto py-12 px-4">
        <div class="bg-white rounded-3xl shadow-xl shadow-indigo-100 overflow-hidden border border-indigo-50">
            <div class="bg-[#764BA2] p-6 sm:p-8 text-white text-center relative overflow-hidden">
                ...
                <h2 class="text-2xl font-bold relative z-10">Create New Assignment</h2>
                <p class="opacity-80 relative z-10 text-sm">Set up a new task and choose the appropriate grading rubric.</p>
            </div>

            <form method="POST" action="{{ route('assignments.store') }}" class="p-6 sm:p-8 space-y-6">
                @csrf
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="title" placeholder="e.g. Mid-term Research Paper" required
                        class="w-full px-4 py-3 rounded-xl border-slate-200 text-slate-700 font-medium focus:border-[#764BA2] focus:ring focus:ring-[#764BA2]/10 transition-all bg-white">
                    @error('title')
