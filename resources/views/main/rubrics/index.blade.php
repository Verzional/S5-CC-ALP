                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            @php
                                                ...
                                                $maxShown = 3;
                                                $shown = array_slice($criteria, 0, $maxShown);
                                                $remaining = count($criteria) - $maxShown;
                                            @endphp
                                            @foreach ($shown as $criterion)
                                                {{ $criterion['name'] }}
                                                ({{ $criterion['weight'] }}%){{ !$loop->last ? ', ' : '' }}
                                            @endforeach
                                            @if ($remaining > 0)
                                                ... and {{ $remaining }} more
                                            @endif
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            ...
                                            <form id="delete-form-{{ $rubric->id }}"
                                                action="{{ route('rubrics.destroy', $rubric) }}" method="POST"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
